...
$(document).ready(function(){
	...
	$.fn.sDispatch = function(config) {
		...
		function execute() {
			if (options.bsf.scope.value == 'this') { return true; }

			if (options.bsf.scope.value == 'all') {
				options.gsf.MT.value = options.bsf.sword.value;
				options.gsf.submit();
			}

			if (options.bsf.scope.value == 'web') {
				options.wsf.MT.value = options.bsf.sword.value;
				options.wsf.submit();
			}
			return false;
		}
	...
	$.fn.fontResizer = function(config){
		var types = ['xx-small','x-small','small','medium','large'],
			...
			defaults = {
				original: 2, // types[2]
				range: 2,
				// 本番環境でdomainがblog.goo.ne.jpのcookieを発行すると
				// [dev/stg].blog.goo.ne.jpのcookieを発行することができなくなるので
				// font-resizer用のcookieは本番環境と共有する
	...
	$.fn.inhebitContext = function(){
		...
		$(bodyimg).each(function(){
			var re = /\.goo\.ne\.jp\/(?:user_image|thumbnail|cnv|image)\//;
	...
	$gbQuery(this).sDispatch({
		id:"global_header_search",
		bsf: document.bsearch,
		gsf: document.gsearch,
		wsf: document.wsearch,
	});
	$gbQuery(this).sDispatch({
		id:"mod_search",
		bsf: document.mod_bsearch,
		gsf: document.mod_gsearch,
		wsf: document.mod_wsearch,
	});
